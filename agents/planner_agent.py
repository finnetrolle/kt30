...
2. **wbs** — структура работ:
   ...
   - duration: длительность фазы в днях (только число)
   ...
   - dependencies: массив идентификаторов задач/пакетов, от которых зависит этот пакет
   - can_start_parallel: boolean - может ли этот пакет начинаться параллельно с другими пакетами той же фазы
   ...
   - dependencies: массив идентификаторов задач, которые должны быть завершены до начала этой задачи
   - can_start_parallel: boolean - может ли эта задача выполняться параллельно с соседними задачами того же пакета

3. **dependencies_matrix** — матрица зависимостей:
   Массив объектов с полями:
   - task_id: идентификатор задачи
   - depends_on: массив идентификаторов задач, от которых зависит
   - parallel_with: массив идентификаторов задач, которые могут выполняться параллельно

4. **risks** — риски проекта:
   Массив объектов с полями:
   - id: идентификатор
...
5. **assumptions** — предположения при планировании

6. **recommendations** — рекомендации по проекту:
   Массив объектов с полями:
   - category: категория
   - priority: приоритет
...
Пример формата ответа:
{
        ...
        "work_packages": [
          {
            ...
            "tasks": [
              ...
              {
                "id": "1.1.2",
                "name": "Анализ нефункциональных требований",
                "description": "Изучение и документирование нефункциональных требований",
                "estimated_hours": 8,
                "duration_days": 1,
                "status": "pending",
                "skills_required": ["Системный аналитик"],
                "dependencies": [],
                "can_start_parallel": true
              }
            ]
  ...
  "dependencies_matrix": [
    {"task_id": "1.1.1", "depends_on": [], "parallel_with": ["1.1.2"]},
    {"task_id": "1.1.2", "depends_on": [], "parallel_with": ["1.1.1"]}
  ],
  "risks": [],
  "assumptions": [],
  "recommendations": []
...
Правила оценки трудозатрат и зависимостей:
...
5. Указывай зависимости между задачами (если задача B не может начаться пока не завершена задача A)
6. Добавляй буфер на риски (обычно +10-15%)
7. duration_days = estimated_hours / 8 (округлить вверх)
8. Отмечай can_start_parallel=true для задач, которые могут выполняться одновременно разными людьми
...
- Зависимости между фазами: следующая фаза обычно зависит от завершения предыдущей
- Внутри фазы некоторые пакеты могут выполняться параллельно"""
